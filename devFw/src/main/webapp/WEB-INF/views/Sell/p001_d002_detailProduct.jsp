...
<script type="text/javascript" charset="utf-8">
													...
													.ajax({
														...
														success : function(
															...
															for (var i = 0; i < data.length; i++) {
																list += '<li><a href="javascript:void(0)" class="faq_open" id="'
																		+ data[i].qna_number
																		+ '" name="'
																		+ data[i].secret_yn
																		+ '">'
																...
																list += '<div class="cont_box">'
																		+ '<span class="inquiry_prod">'
																		+ prod_title
																		+ '</span>';

																if (data[i].secret_yn == 'y') { //비밀글일경우
																	if (memberId == prod_memberId
																			|| memberId == data[i].memberId) { //글 작성자이거나 해당 상품 게시자인 경우
																		list += '<span class="inquiry_text" id="contText" style="font-weight:bold;">'
																				+ data[i].qna_content
																				+ '</span>';
																	} else { //제 3자의 경우
										...
										function() {
											if (loginCheck == null
												...
												if ($(this).attr('name') == 'n') { //관심 상품 등록
													...
													$(this)
															.html(
																	'<img src="${contextPath}/resources/img/detailProduct/fillheart.png" style="width:30px; height:30px;"> 관심 상품  ');
													$(this).css("color",
															"#dd5850");
													...
													$(this)
															.html(
																	'<img src="${contextPath }/resources/img/detailProduct/heart.png" style="width:30px; height:30px;"> 관심 상품  ');
													$(this).css("color",
															"black");
													heart.setAttribute("name",
															"n");
						...
						$(document)
								.on(
										...
										function() {
											...
											var id = $(this).attr("id");
											var writerId = $("#" + id).find(
													"#qna_memberId").val(); //qna 글쓴이

											if (secret == 'y') { //비밀글일 경우
												if (memberId == writerId
														|| memberId == prod_memberId) { //로그인한 아이디가 글쓴이이거나 상품 글쓴이인 경우
													num = $(this).attr("id");
						...
						$(document)
								.on(
										...
										function() {
											...
											var content = $(
													"#prod_inquiry_text").val();
											var product = $("#prod_number")
													.val();
											...
											$
													.ajax({
														...
														success : function(
																responseData) {
															document
																	.getElementById("prod_inquiry_text").value = "";
								...
								.change(
										function() {
											if ($("#secretSelect").is(
													":checked")) { //비밀 글 선택
												document
														.getElementsByName("secret")[0].value = 'y';
											} else {
												document
														.getElementsByName("secret")[0].value = 'n';
											}
										});
	...
	$(window)
			.load(
					function() {
						var auction_yn = document
								.getElementsByName("auction_yn")[0].value;
						...
						if (auction_yn == 'y') {
							...
							setInterval(
									function() {
										...
										var tt = day + ' 일 ' + hour + ' 시간 '
												+ min + " 분 " + second + " 초 ";
										document
												.getElementById("auction_date_cal").innerHTML = tt;
									}, 1000);
						}
					})
				...
				<div class="up">
					...
					<div class="description">
						...
						<c:if test="${product.flea_yn == 'y' }">
							...
							<h3 id="gray-text">
								<a href="./fleaMystore.do?flea_code=${product.fleamarket}">${fleaName }</a><a
									href="./sellerPage.do?memberId=${product.memberId }">(${product.memberId })</a>
							</h3>
						</c:if>
						<c:if test="${product.flea_yn == 'n' }">
							<h3 id="gray-text">
								<a href="./sellerPage.do?memberId=${product.memberId }">${product.memberId }</a>
							</h3>
						</c:if>
						<br>
						<c:choose>
                     ...
                     수량: 
                     <input type="button"
										onClick="abuttonClick('minus')" value="-">
                     ...
                     총 금액: <span id="total"></span>원
							...
							<div id="deliveryTypeSection">
								...
								<div class="successNoticeBox" id="successNoticeBox"
									style="display: none;">
									<div class="arrow_box ">
										장바구니 담기 성공🚀 <br> <br> <input type="button"
											id="noticeButton" value="창닫기" onClick="closeNotice();">&nbsp&nbsp&nbsp
										<input type="button" id="noticeButton" value="장바구니"
											onClick="moveToCart()">
									</div>
								</div>
							...
							<div id="pay">
								...
								<c:if
									...
									<c:if test="${member.getMemberid() != product.memberId }">
										...
										<!--                      <input type="button" class="pay" id="kakaoPay" value="kakaoPay" /> -->
										<input type="button" class="pay" id="message" value="메시지로 문의"
											onClick="location.href='./chatInit.do?prod_number=${product.prod_number }'">
									</c:if>
								...
								<c:if test="${qnaCnt != 0 }">
									<div class="paging_comm" id="paging">
										<c:choose>
											...
											<c:otherwise>
												<c:forEach var="i" begin="1" end="${qnaCnt/10 + 1 }"
													step="1">
