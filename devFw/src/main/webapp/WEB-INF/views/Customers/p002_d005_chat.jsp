			...
			.ready(
					function() {
						...
						function init() {
							...
							ws.onmessage = function(message) {
								...
								if (recData.header == "chat_list") {
									for ( var i in recMessage) {
										messageAdd(
												recMessage[i].contents,
												new Date(
														recMessage[i].send_date),
												recMessage[i].me_at);
									}
								} else if (recData.header == "send_message") {
									updateChat(recData);
									var nowId = $('#chat-header .memberid')
											.text();
									if (nowId == recMessage.sender
											|| nowId == recMessage.receiver) {
										messageAdd(recMessage.contents,
												new Date(recMessage.send_date),
												recMessage.me_at);
									}
							...
							//Add Event
							//유저 선택전  막기
							$("#chat-footer>input[type=button]").prop(
									'disabled', true);
									...
									.on(
											...
											function() {
												if (!$(this)
													...
													$(
															"#chat-footer>input[type=button]")
															.prop('disabled',
																	false);//전송버튼 사용가능
													$('#chat-message').empty();//메시지 창 비우기
													$('.discussion')
															.removeClass(
																	"clicked");//이전에 선택된 노드의 클래스 제거
													$(this).addClass('clicked');//클래스 붙이기

													if ($(this).parent().is(
															'#search-list')) {//search의 discuss일경우 people-list에서도 변경
														var people = $(
																'#people-list>.discussion .memberid')
																.toArray();
														for ( var i in people) {
															if ($(people[i])
																	.text() === $(
													...
													sendText(ws, "chat_list",
															contents);
												}//end if
											});//end discussion event

							//유저 검색
							$("#people-search>input[type=text]").on("change",
									searchMember);

							//메시지 전송
							$("#chat-footer>input[type=button]").on(
									"click",
									function() {
										var textMessage = $(
												'#chat-footer>textarea').val();
										var other = $('#chat-header .memberid')
												.text();
										if (textMessage !== '') {
											var contents = {
												message : textMessage,
						...
						function prependMember(/*memberDivForm*/data) {
							var userList = $('#people-list>.discussion')
									.toArray();
						...
						function updateChat(recData) {
							...
							if (res.me_at == 'true') {//보낸사람이 나면
								divData = new memberDivForm('discussion', null,
										null, res.receiver);
							} else {
								divData = new memberDivForm('discussion',
										recData.body.sender_info.memberid,
										recData.body.sender_info.nickname,
										res.sender);
							}
						...
						function sendText(/*웹소켓 객체*/ws,/*String*/head,/*Object*/
						contents) {
							var result = {
								header : head,
								body : contents
						...
						function popSearchMember(/*회원 정보 객체 배열*/members) {
							...
							if (members.length == 0) {
								...
								for ( var k in members) {
									var mem = members[k];
									console.log(mem);
									var divData = new memberDivForm(
											'pop-member', mem.nickname,
											mem.memberid);
									console.log("divData ")
									console.log(divData);
						...
						function searchMember() {
							...
							if (input === '') {
								...
								$('#search-list').empty();
								var discussions = $(
										'#people-list>div.discussion')
										.toArray();
								for (var j = 0; j < discussions.length; j++) {
									var nick = $(discussions[j]).find(
											'div.nickname').toArray()[0];
									var member = $(discussions[j]).find(
											'div.memberid').toArray()[0];
									if ($(nick).text().indexOf(input) !== -1
											|| $(member).text().indexOf(input) !== -1) {
										var discussClone = discussions[j]
												.cloneNode(true);
						...
						function messageAdd(inputText, date, meCheck) {//메시지,날짜,본인
							...
							//스크롤 밑으로 내리기
							var height = document
									.getElementById('chat-message').scrollHeight;
...
<body>
	<div class="msg-container">
		...
		<div id="chat">
			...
			<div id="chat-message">
				<div id="chat-main">쪽지를 확인할 대상을 선택해주세요</div>
			</div>
			<div id="chat-footer">
				<textarea></textarea>
				<button type="button" class="btn btn-dark" id="btn1">전송</button>
			</div>
		</div>
	</div>
	<div id="user-add">
		<div id="pop-up">
			...
			<div id="pop-search">
				<input type="text" placeholder="새로운 유저 검색" /> <input type="button"
					value="검색">
			</div>
			<div id="pop-list"></div>
			<div id="pop-footer">
