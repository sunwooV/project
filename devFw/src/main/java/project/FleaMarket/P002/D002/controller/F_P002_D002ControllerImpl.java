...
public class F_P002_D002ControllerImpl implements F_P002_D002Controller {
	...
	@Override
	@RequestMapping(value = "/hm/d002/searchInit.do", method = { RequestMethod.GET, RequestMethod.POST })
	...
	@Override
	@RequestMapping(value = "/FleaMarket/P002/D002/searchList.do", method = { RequestMethod.GET, RequestMethod.POST })
	public ModelAndView searchList(@RequestParam(value="p_id", required=false) String p_id, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ModelAndView mav = new ModelAndView("hm/d002_review");
		mav.addObject("searchList", list);
		return mav;
	}

	@Override
	@RequestMapping(value = "/hm/d002/searchMod2.do", method = { RequestMethod.GET, RequestMethod.POST })
	public ModelAndView searchMod(@RequestParam(value="p_mod_id", required=false) String p_id, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List list = d002Service.searchMod(searchMap);
		if(!list.isEmpty()) {
			d002VO = (F_P002_D002VO)list.get(0);
		}
		
		ModelAndView mav = new ModelAndView("hm/p0001_mod");
		mav.addObject("p0001VO", d002VO);
	...
	@Override
	@RequestMapping(value = "/hm/d002/updateMember2.do", method = { RequestMethod.GET, RequestMethod.POST })
	@ResponseBody
	public ResponseEntity updateMember(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			d002Service.updateMember(dataMap);
			
			RequestDispatcher dispatch = request.getRequestDispatcher("/hm/d002/searchList.do");
			dispatch.forward(request, response);
		} catch (Exception e) {
			message = " <script>";
			message += " alert('오류가 발생했습니다. 다시 시도해 주세요');";
			message += " location.href='" + request.getContextPath() + "/hm/d002/searchInit.do'; ";
			message += " </script>";
